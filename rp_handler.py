# rp_handler.py – IsabelaOS Studio v2
# FIXES:
#   1. BASE_VOLUME lee ISE_VOLUME_MOUNT (no hardcodeado)
#   2. SDXL strength=0.06 eliminado → CodeFormer reemplaza
#   3. INSWAPPER_MODEL_PATH corregido
#   4. InsightFace CUDA fallback limpio
 
import os, io, sys, base64, urllib.request, traceback, hashlib
...
from diffusers import (
    FluxPipeline, UniPCMultistepScheduler,
...
def get_realistic_vision():
    ...
    if realistic_pipe: return realistic_pipe
    ...
    try:
        vae = AutoencoderKL.from_pretrained(
            REALISTIC_VAE_ID, torch_dtype=DTYPE_SD15,
            cache_dir=os.environ["HF_HUB_CACHE"],
        )
        realistic_pipe.vae = vae
    except Exception as e:
        print("[IsabelaOS] WARNING RV VAE:", repr(e))
    try:
    ...
    if DEVICE == "cuda":
        ...
        try: realistic_pipe.enable_attention_slicing()
        except Exception: pass
...
def get_codeformer():
    ...
    if _cf_net: return _cf_net, _cf_helper
...
def run_codeformer(img_pil: Image.Image, fidelity_weight: float = 0.90) -> Image.Image:
    """
    fidelity_weight:
      0.90 → standard  (preserva identidad, mínima restauración)
      0.85 → natural   (más restauración, piel más orgánica)
    """
    try:
        ...
        if not helper.cropped_faces:
            print("[IsabelaOS] CodeFormer: no faces found, returning original")
            return img_pil
        ...
        result = helper.paste_faces_to_input_image(upsample_img=None, draw_box=False, face_upsampler=None)
...
def encode_image_jpg(img: Image.Image, quality: int = 92) -> Dict[str, str]:
    ...
    return {"image_b64": b64, "image_data_url": url, "mime": "image/jpeg",
...
def clamp_size(img: Image.Image, max_side=1024) -> Image.Image:
    w, h  = img.size
    scale = min(max_side / max(w, h), 1.0)
    nw, nh = max(int(w*scale//8*8), 256), max(int(h*scale//8*8), 256)
    return img.resize((nw, nh), Image.LANCZOS) if (nw,nh) != (w,h) else img
 
 
def is_flat(img: Image.Image) -> bool:
    try: return np.array(img.convert("RGB"), dtype=np.uint8).std() < 2.0
    except: return False
...
def _ensure_file_from_url(url: str, local_path: str) -> str:
    ...
    tmp = local_path + ".tmp"
    print(f"[IsabelaOS] Downloading: {url[:80]}...")
    ...
    print(f"[IsabelaOS] Cached: {local_path}")
    return local_path
...
def _ort_providers():
    """Intenta CUDA, cae limpio a CPU si libcublasLt no disponible."""
    try:
...
def _apply_identity_lock(image: Image.Image, anchors: List[Image.Image]):
    ...
    try:
        ...
        for i, a in enumerate(anchors):
            ...
            if src_face:
                print(f"[IsabelaOS] Source face in anchor {i+1}")
                break
...
def handle_txt2img(inp: Dict[str, Any]) -> Dict[str, Any]:
    ...
    anchors = []
    if anchor_urls:
        try: anchors = _load_anchor_images(anchor_urls, avatar_id)
        except Exception as e: print("[IsabelaOS] WARNING anchors:", repr(e))
    ...
    print("[pipeline]", {"skin_mode":skin_mode,"cf_fidelity":cf_fidelity,
          "size":[width,height],"anchors":len(anchors),"has_anchor":has_anchor,
          "use_rv_natural":use_rv_natural})
    ...
    if use_rv_natural:
        ...
        rv_steps    = _safe_int(inp.get("natural_rv_steps",24))
        rv_guidance = _safe_float(inp.get("natural_rv_guidance",5.5))
        rv_prompt = (eff +
            ", single person, one woman only, solo portrait, one face only,"
            " realistic portrait, natural skin texture, hyper-detailed pores,"
            " subtle imperfections, unretouched skin, photorealistic skin")
        rv_neg = (neg +
            ", multiple people, duplicate face, extra face, extra head,"
            " plastic skin, smooth skin, airbrushed skin, beauty filter, CGI, anime")
        with torch.inference_mode():
            ctx = torch.autocast("cuda", dtype=DTYPE_SD15) if DEVICE=="cuda" else torch.no_grad()
            with ctx:
                image = pipe(prompt=rv_prompt, negative_prompt=rv_neg,
        ...
        with torch.inference_mode():
            ...
            with ctx:
                image = pipe(prompt=eff, num_inference_steps=steps,
                             width=width, height=height).images[0]
 
    print("[IsabelaOS] Base generation done ✅")
 
    # ── Identity lock ──────────────────────────────────────────────────────
    id_warn = None
    if has_anchor:
        image, id_warn = _apply_identity_lock(image, anchors)
        if id_warn: print("[IsabelaOS] id_warn:", id_warn)
...
def handle_compose_scene(inp: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not fg_b64 or not bg_b64: return {"error":"MISSING_FG_OR_BG"}
    ...
    x1,y1,x2,y2 = cx-nw//2, cy-nh//2, cx-nw//2+nw, cy-nh//2+nh
    bx1,by1 = max(0,x1), max(0,y1)
    bx2,by2 = min(bg_w,x2), min(bg_h,y2)
    if bx2<=bx1 or by2<=by1: return {"error":"PLACEMENT_OUT_OF_BOUNDS"}
    ...
    if cmatch: fg_rgb = _match_color(fg_rgb, bg_roi, fg_a)
 
    if mode == "alpha":
        af = (fg_a.astype(np.float32)/255)[...,None]
        bg_bgr[by1:by2,bx1:bx2] = np.clip(
            fg_rgb.astype(np.float32)*af + bg_roi.astype(np.float32)*(1-af),0,255).astype(np.uint8)
    else:
    ...
    return {**enc, "mode":"compose_scene", "engine":"local_compositor",
