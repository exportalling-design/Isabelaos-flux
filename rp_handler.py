...
# Caches persistentes (Network Volume)
...
def handle_headshot_pro(input_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Headshot Pro SIN prompt del usuario:
    - Sirve para personas y productos
    - Busca look de FOTO DE ESTUDIO (natural), NO revista/publicidad
    - Cambios visibles (antes estaba muy conservador)
    """
    pipe = get_img2img()
    ...
    # 🔥 NUEVO PROMPT (estudio natural, neutro)
    prompt = (
        "professional studio photo, neutral background, soft studio lighting, "
        "balanced exposure, natural tones, high detail, sharp focus, "
        "subtle shadows, realistic color grading, clean composition, "
        "natural look, high quality photography"
    )

    # 🔥 NUEVO NEGATIVE (evita estilo revista y arte)
    negative = (
        "cartoon, anime, illustration, overprocessed, oversharpened, "
        "plastic skin, glamour magazine style, heavy makeup, dramatic lighting, "
        "harsh shadows, watermark, text, blurry, low quality"
    )

    # 🔥 NUEVOS DEFAULTS (más cambio, sin reinventar demasiado)
    steps = int(input_data.get("steps", 28))
    guidance = float(input_data.get("guidance", 6.5))
    strength = float(input_data.get("strength", 0.45))
    ...
    print(
        f"[headshot_pro] size={w}x{h} steps={steps} guidance={guidance} "
...
def handler(event: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if action == "health":
            return {"message": "IsabelaOS worker online (FLUX txt2img + SDXL img2img studio)"}
